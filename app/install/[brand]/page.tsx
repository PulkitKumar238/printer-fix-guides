...
export function generateMetadata({ params }: { params: { brand: string } }): Metadata {
  ...
  return pageMetadata({
    ...
    description: 'Enter your printer model number to continue with guided printer setup assistance.',
...
export default function InstallBrandPage({ params }: { params: { brand: string } }) {
  ...
  const subNav = ['Home', 'Printer setup', 'Connection help', 'Troubleshooting', 'Support'];
    ...
    <div className="bg-white text-[#1a1a1a]">
      <JsonLd
        data={breadcrumbSchema([
          { name: 'Home', url: '/' },
          { name: 'Printer setup', url: '/install' },
        ...
        <div className="mx-auto grid w-full max-w-[90rem] gap-14 px-6 py-16 sm:px-10 sm:py-20 lg:grid-cols-2 lg:gap-20">
          <div>
            <h1 className="font-sans text-4xl font-extrabold leading-tight text-[#111] sm:text-5xl">
              Welcome! Printer Support &amp; Assistance
            </h1>
            ...
            <div className="mt-8">
              <DriverDownload brandName={brand.name} />
            </div>
          </div>
          <div>
      ...
      <div className="bg-[#1a1a1a] py-6 text-center text-sm text-white/70">
        ...
        <p className="mt-1">
          PrinterFix · Independent service, not affiliated with any printer manufacturer.
        </p>
      </div>
    </div>
  );
